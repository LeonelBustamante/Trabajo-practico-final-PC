...
public class Aeropuerto {
    ...
    public synchronized void ingresarAeropuerto(Pasajero pasajero) {
        try {
            while (!estaAbierto) {
                System.out.println(PrintColor.ANSI_BLACK + "[CLASE AEROPUERTO]: "
                        + pasajero + " espera a que el aeropuerto abra." + PrintColor.ANSI_RESET);
                wait();
            }
            System.out.println(PrintColor.ANSI_BLACK + "[CLASE AEROPUERTO]"
                    + pasajero + " se dirige al puesto de atencion." + PrintColor.ANSI_RESET);
            Thread.sleep(2000);
    ...
    public synchronized void comenzarHorarioAtencion() {
        System.out.println(
                PrintColor.ANSI_BLACK + "[CLASE AEROPUERTO] Comenzando horario de atencion." + PrintColor.ANSI_RESET);
        estaAbierto = true;
        notifyAll();
    }
    ...
    public synchronized void terminarHorarioAtencion() {
        estaAbierto = false;
        System.out.println(
                PrintColor.ANSI_BLACK + "[CLASE AEROPUERTO] Termino horario de atencion." + PrintColor.ANSI_RESET);
    }
